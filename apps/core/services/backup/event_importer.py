...
class EventBackupImporter(BaseBackupImporter):
    domain_key = 'EVENTS'
    display_name = 'Events & Workshops'
    description = 'Technical workshops, speaker sessions, bootcamps, and club schedules.'

    required_fields = ['title']

    expected_fields = {
        'title': ['title', 'event title', 'name', 'event name', 'workshop title', 'session'],
        'category': ['category', 'type', 'event type', 'format'],
        'venue': ['venue', 'location', 'hall', 'room', 'auditorium'],
        'start_time': ['start time', 'start_time', 'start date', 'start', 'from', 'date'],
        'end_time': ['end time', 'end_time', 'end date', 'end', 'to'],
        'capacity': ['capacity', 'seats', 'limit', 'max seats', 'max capacity'],
        'description': ['description', 'summary', 'details', 'about', 'overview'],
    }
    ...
    def preview(
        ...
        for idx, r in enumerate(raw_rows, start=1):
            ...
            if not title:
                row_errors.append("Missing required Event title.")

            # Parse start and end time
            start_dt = UserAccountService.parse_legacy_date(normalized_payload.get("start_time")) or timezone.now()
            end_dt = UserAccountService.parse_legacy_date(normalized_payload.get("end_time")) or (start_dt + timedelta(hours=2))

            # Deduplication Strategy: title + start_time
            matched_event = None
            if title:
                matched_event = Event.objects.filter(title__iexact=title, start_time__date=start_dt.date()).first()
                if not matched_event:
                    matched_event = Event.objects.filter(title__iexact=title).first()
            ...
            import_row_objects.append(
                ImportRow(
                    ...
                    target_object_id=str(matched_event.id) if matched_event else "",
                )
            )
    ...
    def commit(
        ...
        with transaction.atomic():
            for row in rows:
                ...
                try:
                    ...
                    start_dt = UserAccountService.parse_legacy_date(p.get("start_time")) or timezone.now()
                    end_dt = UserAccountService.parse_legacy_date(p.get("end_time")) or (start_dt + timedelta(hours=2))
                    category = p.get("category", "Workshop").strip() or "Workshop"
                    venue = p.get("venue", "Campus Auditorium").strip() or "Campus Auditorium"
                    desc = p.get("description", "").strip() or f"{title} organized by SRKR Coding Club."
                    
                    try:
                        capacity = int(p.get("capacity", 100))
                    except (ValueError, TypeError):
                        capacity = 100

                    # Deduplicate by title + start_time
                    event = Event.objects.filter(title__iexact=title, start_time__date=start_dt.date()).first()
                    created = False
                    ...
                    row.target_object_id = str(event.id)
                    row.save(update_fields=['target_object_id'])
                    processed_events.append((event, created))
        ...
        return {
            "success": True,
            "imported_count": len(processed_events),
            "inserted_count": sum(1 for _, c in processed_events if c),
            "updated_count": sum(1 for _, c in processed_events if not c),
        }
