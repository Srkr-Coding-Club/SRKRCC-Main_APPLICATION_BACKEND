...
class FormAutomationService:
    ...
    @classmethod
    def dispatch_confirmation_email(cls, form, resolved_user, answers_by_field_id: dict) -> None:
        """
        Sends the form's confirmation_email_template to the submitter. Must be called
        AFTER the response's transaction has committed — never from inside an open
        transaction (mirrors MemberImportService.commit_import's existing convention)
        so a failed send can never roll back an otherwise-successful submission.
        Swallows and logs its own errors for the same reason.
        """
        if not form.confirmation_email_enabled or not form.confirmation_email_template:
            return

        # The whole method is best-effort: a misconfigured field mapping or a
        # send failure must never surface as an error on what was already a
        # successful submission (the Response is already committed by this point).
        try:
            if resolved_user:
